...
const UpdateArtistsModal = () => {
  const uploadModal = useUpdateArtistsModal();
  ...
  const userId = uploadModal.artistId;
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (values) => {
    try {
      ...
      if (photoFile && !name) {
        ...
        await updateArtist(userId || '', undefined, image, token);
      }
      else if (name && !photoFile) {
        await updateArtist(userId || '', name, undefined, token);
      }
      else if (name && photoFile) {
        photoUploadResponse = await handlePhotoSubmit({ photo: photoFile });
        image = photoUploadResponse.url;
        await updateArtist(userId || '', name, image, token);
      }
  ...
  return (
    <Modal
      ...
      description="Complete los detalles a continuación para actualizar al artista."
...
export default UpdateArtistsModal;
